...
export default function TypesLieSwatter() {
	return (
		<div className="animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-5xl mx-auto p-4 sm:p-6">
			...
			<header className="mb-16">
				...
				<h1 className="text-4xl sm:text-5xl font-extrabold text-white tracking-tight mb-6">
					Types
					<span className="text-transparent bg-clip-text bg-linear-to-r from-purple-400 to-pink-400">LieSwatter</span>
				</h1>
				<p className="text-lg text-gray-400 leading-relaxed max-w-2xl">
					Configuration options for the Lie Swatter minigame.
				</p>
			</header>
			...
			<section className="mb-20">
				...
				<div className="border border-white/10 rounded-xl overflow-hidden shadow-2xl">
					<CodeBlock
						className="my-0!"
						language="typescript"
						code={`export interface LieSwatterTypes {
    context: Context;
			...
			<section className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-20">
				...
				<div className="lg:col-span-2 rounded-2xl border border-white/10 bg-[#0d0d0e] overflow-hidden">
					...
					<div className="divide-y divide-white/5">
						...
						<PropRow
							name="buttons"
							type="{ true: string, lie: string }"
							desc="Custom labels for the True and Lie buttons."
							defaultVal='{ true: "Truth", lie: "Lie" }'
				...
				<div className="rounded-xl border border-white/10 bg-[#0d0d0e] p-6 flex flex-col items-center justify-center text-center">
					<Bug className="w-12 h-12 text-purple-500/20 mb-4" />
					<h3 className="font-bold text-white mb-2">API Source</h3>
					<p className="text-xs text-gray-500">
						This game automatically connects to the OpenTDB API. No questions list is required!
					</p>
				</div>
			</section>
			...
			<section className="mb-20">
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
					...
					<StatusCard
						...
						title="Failure"
						items={[
							{ key: "loseMessage", desc: "Sent on wrong guess." },
							{ key: "timesUpMessage", desc: "Sent when time expires." },
						]}
					/>
					...
					<StatusCard
						...
						color="blue"
						title="States"
						items={[{ key: "thinkMessage", desc: "Shown while fetching data." }]}
					/>
					...
					<StatusCard
						...
						title="Errors"
						items={[
							{ key: "errors.failedFetch", desc: "API fetch failure." },
					...
					<table className="w-full text-left text-sm text-gray-400">
						...
						<tbody className="divide-y divide-white/5">
							<tr>
								...
								<td className="p-4">
									<code className="bg-white/10 px-1 rounded">{"{{answer}}"}</code> ("Truth" or "Lie"){" "}
									<code className="bg-white/10 px-1 rounded">{"{{time}}"}</code>
								</td>
							</tr>
							<tr>
								<td className="p-4 font-mono text-purple-300">loseMessage</td>
								<td className="p-4">
									<code className="bg-white/10 px-1 rounded">{"{{answer}}"}</code>
								</td>
							</tr>
							<tr>
								<td className="p-4 font-mono text-purple-300">states.active / won / lost</td>
								<td className="p-4">
									<code className="bg-white/10 px-1 rounded">{"{{gameTitle}}"}</code>{" "}
									<code className="bg-white/10 px-1 rounded">{"{{text}}"}</code> (The question)
								</td>
							</tr>
							<tr>
