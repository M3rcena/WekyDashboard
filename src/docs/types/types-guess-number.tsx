			...
			<section className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-20">
				...
				<div className="rounded-xl border border-white/10 bg-[#0d0d0e] p-6 flex flex-col items-center justify-center text-center">
					...
					<p className="text-xs text-gray-500">
						If you don't set the <code>number</code> property, the game will automatically pick a random number between{" "}
						<strong>1 and 1000</strong>.
					</p>
				</div>
			</section>
				...
				<div className="overflow-hidden rounded-xl border border-white/10 bg-[#0d0d0e]">
					<table className="w-full text-left text-sm text-gray-400">
						...
						<tbody className="divide-y divide-white/5">
							<tr>
								<td className="p-4 font-mono text-purple-300">winMessage.publicGame</td>
								<td className="p-4">
									<code className="bg-white/10 px-1 rounded">{"{{number}}"}</code>{" "}
									<code className="bg-white/10 px-1 rounded">{"{{winner}}"}</code> (ID){" "}
									<code className="bg-white/10 px-1 rounded">{"{{time}}"}</code>{" "}
									<code className="bg-white/10 px-1 rounded">{"{{totalparticipants}}"}</code>
								</td>
							</tr>
							<tr>
								<td className="p-4 font-mono text-purple-300">winMessage.privateGame</td>
								<td className="p-4">
									<code className="bg-white/10 px-1 rounded">{"{{number}}"}</code>{" "}
									<code className="bg-white/10 px-1 rounded">{"{{time}}"}</code>
								</td>
							...
							<tr>
								...
								<td className="p-4">
									...
									<code className="bg-white/10 px-1 rounded">{"{{time}}"}</code>{" "}
									<code className="bg-white/10 px-1 rounded">{"{{hintRange}}"}</code>
								</td>
							</tr>
							<tr>
								<td className="p-4 font-mono text-purple-300">states.higher / lower</td>
								<td className="p-4">
									<code className="bg-white/10 px-1 rounded">{"{{gameTitle}}"}</code>{" "}
									<code className="bg-white/10 px-1 rounded">{"{{guess}}"}</code>{" "}
									<code className="bg-white/10 px-1 rounded">{"{{currentRange}}"}</code>
								</td>
							</tr>
						</tbody>
					</table>
