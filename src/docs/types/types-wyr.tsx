...
export default function TypesWouldYouRather() {
	return (
		<div className="animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-5xl mx-auto p-4 sm:p-6">
			...
			<header className="mb-16">
				...
				<p className="text-lg text-gray-400 leading-relaxed max-w-2xl">
					Configuration options for the Would You Rather minigame.
				</p>
			</header>
			...
			<section className="mb-20">
				...
				<div className="border border-white/10 rounded-xl overflow-hidden shadow-2xl">
					<CodeBlock
						className="my-0!"
						language="typescript"
						code={`export interface WouldYouRatherTypes {
    context: Context;
    ...
    // ... custom messages
			...
			<section className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-20">
				...
				<div className="lg:col-span-2 rounded-2xl border border-white/10 bg-[#0d0d0e] overflow-hidden">
					...
					<div className="divide-y divide-white/5">
						...
						<PropRow
							...
							desc="Custom labels for the 'Option A' and 'Option B' buttons."
							defaultVal='{ optionA: "Option A", optionB: "Option B" }'
				...
				<div className="rounded-xl border border-white/10 bg-[#0d0d0e] p-6 flex flex-col items-center justify-center text-center">
					<Split className="w-12 h-12 text-blue-500/20 mb-4" />
					<h3 className="font-bold text-white mb-2">API Connection</h3>
					<p className="text-xs text-gray-500">
						Scenarios and stats are automatically retrieved from the Weky API network.
					</p>
				</div>
			</section>
			...
			<section className="mb-20">
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
					<StatusCard
						...
						color="green"
						title="Success"
						items={[{ key: "states.result", desc: "Shown after user votes." }]}
					/>
					...
					<StatusCard
						...
						title="States"
						items={[
							{ key: "thinkMessage", desc: "Shown while fetching." },
						...
						<tbody className="divide-y divide-white/5">
							<tr>
								...
								<td className="p-4">
									...
									<code className="bg-white/10 px-1 rounded">{"{{option1}}"}</code>{" "}
									<code className="bg-white/10 px-1 rounded">{"{{option2}}"}</code>
								</td>
							</tr>
							<tr>
								<td className="p-4 font-mono text-purple-300">states.result</td>
								<td className="p-4">
									<code className="bg-white/10 px-1 rounded">{"{{gameTitle}}"}</code>{" "}
									<code className="bg-white/10 px-1 rounded">{"{{option1}}"}</code>{" "}
									<code className="bg-white/10 px-1 rounded">{"{{option2}}"}</code>{" "}
									<code className="bg-white/10 px-1 rounded">{"{{stats1}}"}</code>{" "}
									<code className="bg-white/10 px-1 rounded">{"{{stats2}}"}</code>{" "}
									<code className="bg-white/10 px-1 rounded">{"{{userChoice}}"}</code>
								</td>
							</tr>
							<tr>
