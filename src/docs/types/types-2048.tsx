				...
				<ul className="space-y-4">
					{items.map((item, idx) => (
						<li key={idx} className="group/item">
							<div className="font-mono text-sm text-white mb-1 group-hover/item:text-purple-300 transition-colors">
								{item.key}
							</div>
...
export default function Types2048() {
		...
		<div className="animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-5xl mx-auto p-4 sm:p-6">
			...
			<header className="mb-16">
				...
				<h1 className="text-4xl sm:text-5xl font-extrabold text-white tracking-tight mb-6">
					Types<span className="text-transparent bg-clip-text bg-linear-to-r from-purple-400 to-pink-400">2048</span>
				</h1>
				<p className="text-lg text-gray-400 leading-relaxed max-w-2xl">
					Configuration interface for the 2048 minigame. Customize emojis, messages, and timeout settings.
				</p>
			</header>
    ...
    // ... messages (see below)
			...
			{/* CORE SETTINGS */}
			<section className="mb-20">
				<div className="rounded-2xl border border-white/10 bg-[#0d0d0e] overflow-hidden">
					<div className="px-6 py-4 border-b border-white/5 bg-white/2 flex items-center justify-between">
						<div className="flex items-center gap-2">
							<Palette className="w-4 h-4 text-purple-400" />
							<h3 className="text-sm font-bold text-white">Core Properties</h3>
						</div>
					</div>
					<div className="divide-y divide-white/5">
						...
						<PropRow
							name="embed"
							type="Partial<Embeds>"
							desc="Customize the title and color of the game embed."
							defaultVal='{ title: "2048", color: 0x5865f2 }'
							icon={Palette}
						/>
						<PropRow
							name="time"
							...
							defaultVal="600000 (10 min)"
							icon={Clock}
						/>
						<PropRow
							name="emojis"
							type="{ up: string; down: string; left: string; right: string; }"
							desc="Overrides for the button emojis."
							defaultVal='{ up: "⬆️", down: "⬇️", left: "⬅️", right: "➡️" }'
							icon={Gamepad2}
						/>
					</div>
				</div>
			...
			<section className="mb-20">
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
					<StatusCard
						...
						title="Status"
						items={[
							{ key: "loadingMessage", desc: "Shown while initializing." },
							{ key: "activeMessage", desc: "Shown while playing." },
						]}
					/>
					<StatusCard
						icon={CheckCircle2}
						...
						title="End Game"
						items={[
							{ key: "wonMessage", desc: "Shown on victory (2048 tile)." },
							{ key: "gameoverMessage", desc: "Shown when grid is full." },
						]}
					/>
					<StatusCard
						icon={AlertCircle}
						...
						title="Stops"
						items={[
							{ key: "quitMessage", desc: "Shown on manual quit." },
							{ key: "timeoutMessage", desc: "Shown when time expires." },
						]}
					/>
					<StatusCard
						icon={XCircle}
